...
class MainWindow(Qt.QMainWindow):
    ...
    def cubic_skeleton(self):
        ...
        #self.plotter.add_text_slider_widget(self.max_cube_ray, ['01 rays','15 rays', '20 rays'], value=0)
        # self.plotter.add_text_slider_widget(self.next_cubes_ray, ['01 rays','15 rays', '20 rays'], value=0)
        # primary cube
        starting_face_center, starting_face_normal = self.max_cube_ray()
        # secondary cubes
        # face_center_set, face_normal_set = self.next_cubes_ray(starting_face_center, starting_face_normal)
        # print(face_center_set)
        # tertiary cubes
        # face_center_set_1,face_normal_set_1 = self.tertiary_cubes_ray(face_center_set[0], face_normal_set[0])
        # face_center_set_2, face_normal_set_2 = self.tertiary_cubes_ray(face_center_set[1], face_normal_set[1])
    ...
    def max_cube_ray(self):
        ...
        global max_cube_vol, max_cube
        global max_cube_start, max_cube_end, max_cube_run
        global top_rays, top_ints, bottom_rays, bottom_ints, face_center, max_normal
        ...
        top_vert, top_rays, top_ints = self.cube_center_ray(Vol_centroid, 'z')
        top = self.furthest_pt(top_vert, Vol_centroid)

        # find the nearest possible cube vertex from bottom rays & mesh intersection
        bottom_vert, bottom_rays, bottom_ints = self.cube_center_ray(Vol_centroid, '-z')
        ...
        max_cube = self.plotter.add_mesh(pv.PolyData(max_cube_V, max_cube_F), show_edges=True, line_width=3, color="r", opacity=0.6)
    ...
    def cube_center_ray(self, start, dir):
        ...
        r_rot = np.pi/2
        r_num = 1
        # r_dec = -2*np.pi/r_num       #could include in the future
        r_dec = 0
        l_wid = 5
        pt_size = 20
        ...
        for i in range(0, r_num):
            for j in range(0, 4):
                ...
                # rays[1] = self.plotter.add_mesh(pv.Line(Vol_centroid, r_end[1]), color='w', line_width=l_wid)
                # ints[0] = self.plotter.add_mesh(pv.PolyData(r_pts[0]), color='w', point_size=pt_size)
    ...
    def create_cube(self, vertex, starting_pt, axis):
        ...
        V_1 = np.array(vertex)
        self.plotter.add_mesh(pv.PolyData(V_1), color="y", point_size=10.0, render_points_as_spheres=True)
        ...
        # pv.Plane(center=face_center,direction = max_normal, i_size=1,j_size=1)
    ...
    def next_cubes_ray(self, face_center, face_normal):
        ''' create cubes within the mesh from the face centers of the first cube'''
        global next_cube_vol
        global next_rays, next_ints, next_cubes, next_cube_vol_sum , next_cubes, next_cube_face_centers
        ...
        next_cube_vol_sum = 0
        next_cubes = [0] * 6
        next_cube_face_center = [[0] *3] * 6
        next_cube_face_normal = [[0] *3] * 6
        next_rays = [0] * 6 * r_num
        next_ints = [0] * 6 * r_num
        print(face_center)
        # fix face_normal
        normal = face_center[0] - Vol_centroid
        if (np.sign(normal[2]) != np.sign(face_normal[0,2])):
            face_normal =  np.negative(face_normal)

        # loop through all 6 faces of max cube
        for i in range(0, 6):
            # create rotaional matrix about max cube normals
            R = self.rot_axis(face_normal[i])
            ...
            for j in range(0, r_num):
                for k in range(0, 4):
                    if k == 0:
                        if (i == 0) or (i == 5):
                            r_dir[0] = np.array([np.sqrt(2)/2 * np.cos(np.pi/4 + r_dec * j), np.sqrt(2)/2 * np.sin(np.pi/4 + r_dec * j), face_normal[i][2]])
                        else:
                            x,y = sp.symbols('x,y')
                            f = sp.Eq(face_normal[i][0]*x + face_normal[i][1]*y, 0)
                            g = sp.Eq(x**2 + y**2, 0.5**2)
                            inc = sp.solve([f,g],(x,y))
                            # r_dir[0] = np.zeroes(3,1)
                            r_dir[0] = np.array(face_normal[i] + [inc[0][0], inc[0][1], 0.5])
                        r_dir_norm[0] = r_dir[0] / np.linalg.norm(r_dir[0])
            ...
            next_cube_V, next_cube_F, next_cube_vol = self.create_cube(face[2][face[1],:], face_center[i], face_normal[i])
            next_cubes[i] = self.plotter.add_mesh(pv.PolyData(next_cube_V, next_cube_F), show_edges=True, line_width=3, color="g", opacity=0.6)
            next_cube_face_center[i] = np.array(pv.PolyData(next_cube_V, next_cube_F).cell_centers().points)
            next_cube_face_normal[i] = pv.PolyData(next_cube_V, next_cube_F).cell_normals
            self.plotter.add_mesh(next_cube_face_center[i], color="b", point_size=8.0, render_points_as_spheres=True)
            # print(next_cube_V)
            # next cube volume
        ...
        return next_cube_face_center, next_cube_face_normal

    def tertiary_cubes_ray(self, next_cube_face_center, next_cube_face_normal):
        ''' create cubes within the mesh from the face centers of the first cube'''
        global next_cube_vol
        global next_rays, next_ints, next_cube_V, next_cube_F, next_cube_vol_sum , Vol_centroid
        ...
        next_cube_vol_sum = 0
        next_cubes = [0] * 6
        # next_cube_face_center = [0] * 6
        # next_cube_face_normal = [0] * 6
        next_cube_face_center_2 = [0] * 6
        next_cube_face_normal_2 = [0] * 6

        # print(next_cube_face_center)
        # face_normal = [0] * 6
        next_rays = [0] * 6 * r_num
        next_ints = [0] * 6 * r_num
        
        # fix face_normal
        normal = next_cube_face_center[0] - Vol_centroid
        if (np.sign(normal[2]) != np.sign(next_cube_face_normal[0,2])):
            next_cube_face_normal =  np.negative(next_cube_face_normal)

        # loop through all 6 faces of max cube
        for i in range(0, 2):
            # create rotaional matrix about max cube normals
            R = self.rot_axis(next_cube_face_normal[i])
            ...
            for j in range(0, r_num):
                for k in range(0, 4):
                    if k == 0:
                        if (i == 0) or (i == 5):
                            r_dir[0] = np.array([np.sqrt(2)/2 * np.cos(np.pi/4 + r_dec * j), np.sqrt(2)/2 * np.sin(np.pi/4 + r_dec * j), next_cube_face_normal[i][2]])
                        else:
                            x,y = sp.symbols('x,y')
                            f = sp.Eq(next_cube_face_normal[i][0]*x + next_cube_face_normal[i][1]*y, 0)
                            g = sp.Eq(x**2 + y**2, 0.5**2)
                            inc = sp.solve([f,g],(x,y))
                            r_dir[0] = np.array(next_cube_face_normal[i] + [inc[0][0], inc[0][1], 0.5])
                            # r_dir[0] = np.array(next_cube_face_normal[i] + [np.negative(next_cube_face_normal[i][0])*np.rint(next_cube_face_center[i][0])*1, np.negative(next_cube_face_normal[i][1])*np.rint(next_cube_face_center[i][1])*1, 1])
                        r_dir_norm[0] = r_dir[0] / np.linalg.norm(r_dir[0])
                        r_end[0] = next_cube_face_center[i] + r_dir_norm[0] * r_len
                        r_end[0] = np.dot(R(j*r_dec), (r_end[0]-Vol_centroid).T).T
                        if i == 1:
                            print(next_cube_face_normal[i])
                            print(next_cube_face_center[i])
                            # print(face_center[i])
                    else:
                        r_end[k] = np.dot(R(k*r_rot), (r_end[0]-Vol_centroid).T).T
                        r_end[k] = r_end[k] + Vol_centroid
                    ...
                    # perform ray trace
                    r_pts, r_ind = mesh.ray_trace(next_cube_face_center[i], r_end[k])

                    # show rays
                    next_rays[i*r_num] = self.plotter.add_mesh(pv.Line(next_cube_face_center[i], r_end[k]), color='w', line_width=l_wid)
                    next_ints[i*r_num] = self.plotter.add_mesh(pv.PolyData(r_pts[0]), color='w', point_size=pt_size)
                    # next_rays[i*r_num+k] = self.plotter.add_mesh(pv.Line(next_cube_face_center[i], r_end[k]), color='w', line_width=l_wid)
                    # next_ints[i*r_num+k] = self.plotter.add_mesh(pv.PolyData(r_pts[0]), color='w', point_size=pt_size)
                ...
                # find nearest vertice among the ray intersections
                r_int = np.reshape(r_int, (4,3))
                ori_nearest, ori_p, ori_V = self.nearest_pt(r_int, next_cube_face_center[i])
                r_int = []
                ori_r_int = np.append(ori_r_int, ori_V[ori_p,:])

            ori_r_int = np.reshape(ori_r_int, (r_num,3))
            face = self.furthest_pt(ori_r_int, next_cube_face_center[i])

            # create cube from nearest vertice
            next_cube_V, next_cube_F, next_cube_vol = self.create_cube(face[2][face[1],:], next_cube_face_center[i], next_cube_face_normal[i])
            print(next_cube_face_center[1])
            # print(next_cube_face_normal[1])
            next_cubes[i] = self.plotter.add_mesh(pv.PolyData(next_cube_V, next_cube_F), show_edges=True, line_width=3, color="y", opacity=0.6)
            next_cube_face_center_2[i] = np.array(pv.PolyData(next_cube_V, next_cube_F).cell_centers().points)
            next_cube_face_normal_2[i] = pv.PolyData(next_cube_V, next_cube_F).cell_normals
            self.plotter.add_mesh(next_cube_face_center[i], color="b", point_size=8.0, render_points_as_spheres=True)
            # print(next_cube_V)
            # next cube volume
            next_cube_vol_sum = next_cube_vol_sum + next_cube_vol
        ...
        return next_cube_face_center_2, next_cube_face_normal_2
    
    # def quatenary_cubes_ray(self, face_center, face_normal)


    #voxelize the mesh
    def voxelize_model(self,value):
        voxelize(mesh,64,solid=True)
        #trimesh.voxel.VoxelGrid
        #voxels = pv.voxelize(mesh, density=mesh.length/200)
        #self.plotter.add_mesh(voxels, color=True, show_edges=True, opacity=0.5)
        #self.plotter.add_mesh(mesh, color="lightblue", opacity=0.5)
